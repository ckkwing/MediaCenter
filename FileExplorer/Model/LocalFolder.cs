...
namespace FileExplorer.Model
{
    public class LocalFolder : FolderBase
    {
        ...
        static LocalFolder()
        {
            ...
            //Add special excluded folder
            AddAdditionalExcludedFolder();
        }

        private static void AddAdditionalExcludedFolder()
        {
            int length = Environment.SystemDirectory.Length;
            string systemDiskSpace = Environment.SystemDirectory.Remove(3, length - 3);

            string excludedBT = systemDiskSpace + @"$Windows.~BT";
            excludedBT = excludedBT.ToLower();
            if (!excludedBT.IsNullOrEmpty() && !LocalExcludedFolders.Contains(excludedBT))
            {
                LocalExcludedFolders.Add(excludedBT);
            }

            string excludedWS = systemDiskSpace + @"$Windows.~WS";
            excludedWS = excludedWS.ToLower();
            if (!excludedWS.IsNullOrEmpty() && !LocalExcludedFolders.Contains(excludedWS))
            {
                LocalExcludedFolders.Add(excludedWS);
            }

            string excludedTR = systemDiskSpace + @"$INPLACE.~TR";
            excludedTR = excludedTR.ToLower();
            if (!excludedTR.IsNullOrEmpty() && !LocalExcludedFolders.Contains(excludedTR))
            {
                LocalExcludedFolders.Add(excludedTR);
            }

            string excludedQ = systemDiskSpace + @"$WINDOWS.~Q";
            excludedQ = excludedQ.ToLower();
            if (!excludedQ.IsNullOrEmpty() && !LocalExcludedFolders.Contains(excludedQ))
            {
                LocalExcludedFolders.Add(excludedQ);
            }

            string excludedOLD = systemDiskSpace + @"Windows.Old";
            excludedOLD = excludedOLD.ToLower();
            if (!excludedOLD.IsNullOrEmpty() && !LocalExcludedFolders.Contains(excludedOLD))
            {
                LocalExcludedFolders.Add(excludedOLD);
            }
        }
